...
class PrimeAlgorithmTests extends JUnitSuite with AssertionsForJUnit {
  ...
  def testAlgorithmDetectsPrime() {
    assert(ListBasedAlgorithm.isPrime(IsPrime(somePrimes, 23)))
  }
  
  @Test
  def testAlgorithmDetectsNotPrime() {
    assert(!ListBasedAlgorithm.isPrime(IsPrime(somePrimes, 25)))
  }
